...
export class AdvertsNotPremiumService {
  ...
  constructor() { 
    ...
    },
    {
        ...
        location: "Santiago, San Bernardo"
    }
    ]
  ...
  async getAdvertsFromStorage(): Promise<Advert[]> {
    ...
    const storedAdverts = ret.value ? JSON.parse(ret.value) : [];  // Verifica si ret.value es null
    return storedAdverts;
  }
  ...
  async saveAdvertsIntoStorage(): Promise<boolean> {
    try {
      ...
      console.error('Error saving adverts to storage', error);
      return false;
    }
  }
}